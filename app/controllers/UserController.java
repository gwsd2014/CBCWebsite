...
public class UserController extends Controller {
	...
	public static Result login() {
		...
		Form<AuthenticationData> loginForm = form(AuthenticationData.class).bindFromRequest();
	...
	public static Result createStudent_submit() {
		...
		Form<RegistrationData> createUserForm = form(RegistrationData.class).bindFromRequest();
		if (createUserForm.hasErrors()) {
			return badRequest(createStudent.render(createUserForm, "Student"));
		} else {
			User newUser = User.create(new User(createUserForm.get(), "Student"));
	...
	public static Result editStudent(Long userId) {
		...
		RegistrationData data = new RegistrationData(student.email, student.username, student.password, student.password, student.firstName, student.lastName, student.grade, userId);
	...
	public static Result editStudent_submit(Long userId) {
		...
		Form<RegistrationData> editStudentForm = form(RegistrationData.class).bindFromRequest();
		if (editStudentForm.hasErrors()) {
			...
			return ok(viewStudent.render(Class.allInstrClasses(user, true), Class.getStudentClasses(user, student),student));
		}
	}
	...
	public static Result createInstructor_submit() {
		...
		if (createUserForm.hasErrors()) {
			...
			if (picture == null) {
				flash("error", "No user image specified! Please provide one, then try again.");
				return badRequest(register.render(createUserForm, "Instructor"));
			}
	        ...
	        try {
	            ...
	            File convertedFile = new File("public/images/users", newUser.id.toString() + ".png");
	...
	public static Result editInstructor() {
		...
		RegistrationData data = new RegistrationData(user.email, user.username, user.password, user.password, user.firstName, user.lastName, user.grade, user.id);
		Form<RegistrationData> form = Form.form(RegistrationData.class);
		form = form.fill(data);
	...
	public static Result editInstructor_submit() {
		...
		Form<RegistrationData> editUserForm = form(RegistrationData.class).bindFromRequest();
		if (editUserForm.hasErrors()) {
			...
			return ok(index.render());	// TODO: Perhaps redirect to some kind of private profile where instructors can just view their registration details, not edit them? (like we already have for classes)
		}
	}
