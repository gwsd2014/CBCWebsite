...
import views.html.browseClasses;
import views.html.classStudents;
import views.html.createClass;
import views.html.editClass;
import views.html.enrollStudents;
import views.html.viewClass;
...
public class ClassController extends Controller {
	...
	public static Result create_submit() {
		...
		Form<ClassData> createClassForm = form(ClassData.class)
				.bindFromRequest();
		if (createClassForm.hasErrors()) {
			return badRequest(createClass.render(createClassForm));
		} else {
			Class newClass = new Class(createClassForm.get(),
					User.byId(session("userId")));
			...
			if (picture == null) {
				flash("error",
						"No class image specified! Please provide one, then try again.");
				return badRequest(createClass.render(createClassForm));
			}
			...
			try {
				...
				File convertedFile = new File("public/images/classes",
						newClass.id.toString() + ".png");
	...
	public static Result edit(Long classId) {
		...
		ClassData data = new ClassData(classs.name, classs.description,
				String.valueOf(classs.gradeLow),
				String.valueOf(classs.gradeHigh));
	 ...
	 */
	public static Result browse(int page, String sortBy, String order,
			String filter) {
		User user = User.byId(session("userId"));
		if (user == null) {
			return redirect(routes.UserController.index(request().uri()));
		...
		return ok(browseClasses.render(models.Class.getClasses(user, false),
				models.Class.page(user, page, 10, sortBy, order, filter, true),
				sortBy, order, filter));
	}
	...
	public static Result enrollStudents(Long classId, boolean restrictByGrade) {
		...
		if (restrictByGrade) {
			return ok(enrollStudents.render(
					User.getStudents(classs.gradeLow, classs.gradeHigh),
					classs, restrictByGrade));
		} else {
			return ok(enrollStudents.render(User.allStudents(), classs,
					restrictByGrade));
		}
	}
